...
function hasClass(element, newClass) {
  return element.className.match(new RegExp('(\\s|^)'+newClass+'(\\s|$)'));
}
...
function setUpToggles($scroller) {
  var toggleIDs = ["context-box-info-toggle", 
                   "context-box-links-toggle"];
  var contentIDs = ["context-box-info", 
                    "context-box-links"];
  ...
  for (var i = 0; i < toggleIDs.length; i++) {
    ...
    element.addEventListener("click", getToggleListener(i, toggleIDs, contentIDs, $scroller))
  }
}
